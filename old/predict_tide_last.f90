...
!cc   
      ...
      integer, allocatable:: tmask(:,:,:,:),mask(:),x_idx(:),y_idx(:),&
                             time_mjs(:)
      real xt,yt
      real*8, allocatable:: time_mjd(:) !, time_mjs(:)
...
!     ASCII OUTFILE HEADER:
      ...
      status = nf_def_dim(ncid_new, 'time_counter', ntime, dimid_time)
      ! Set vars
      status = nf_def_var(ncid_new, 'time_counter', nf_char, 1, dimid_time, &
               varid_time)
      status = nf_put_att_text(ncid_new, varid_time,'units',33,&
               'seconds since 1994-12-24 06:28:16') ! 1994-12-24 06:28:16 1858-11-17
      write(6,*)'Time status.. ',status
...
! OB usage March 2011 (the other case has been removed)
      ...
      !coo_array=
      write (syyyy, "(I8)") yyyy(1)
      write(*,*)'Prova..',syyyy
      time_array=syyyy//'-'!//mm//'-'//dd//' '//hh//':'//mi//':'//ss
      ...
      write(6,*)'Time status.. ',status,time_mjs(1),time_mjs(720)
      status = nf_put_var(ncid_new, varid_navlat,nav_lat)
      status = nf_put_var(ncid_new, varid_navlon,nav_lon)
      status = nf_put_var(ncid_new, varid_field, tides_z)
      ! Close the outfile
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 time_mjd(ntime)!,time_mjs(ntime)
